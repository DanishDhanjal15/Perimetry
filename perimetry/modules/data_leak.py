...
def banner():
    ...
    =============================================
           Perimetry - Advanced Data Leak Checker
    =============================================
    """)
...
def check_email_breaches(email: str, session: requests.Session) -> tuple[str, list[dict] | None, str | None]:
    ...
    headers = {"User-Agent": "PerimetryDataLeakChecker/1.0"}
    try:
...
def main():
    banner()
    p = argparse.ArgumentParser("Perimetry – Advanced Data Leak Checker")
    p.add_argument("domain", help="Domain to check")
    # The runner always invokes modules as `<domain> <threads> [opts-json]`, so
    # accept those positionally too - not just as the --threads flag.
    p.add_argument("threads_pos", nargs="?", type=int, default=None, help=argparse.SUPPRESS)
    p.add_argument("opts_json", nargs="?", default=None, help=argparse.SUPPRESS)
    ...
    threads = args.threads_pos if args.threads_pos else args.threads
    domain = clean_domain_input(args.domain)
    emails = args.email if args.email else generate_emails(domain)
    console.print(Fore.WHITE + f"[*] Checking {len(emails)} address(es) on {domain} with {threads} threads\n")
