...
from perimetry.utils.util import resolve_to_ip, clean_domain_input, ensure_directory_exists, write_to_file
from perimetry.config.settings import DEFAULT_TIMEOUT, RESULTS_DIR, EXPORT_SETTINGS, API_KEYS
...
def banner():
    ...
    console.print("[cyan]     Perimetry – Geo-IP Spoof Detection")
    console.print(f"[#2EC4B6]{bar}\n")

def query(name, tpl, ip, key, timeout):
    headers = {"User-Agent": "Perimetry"}
    if key:
...
def run(target, threads, opts):
    ...
    if EXPORT_SETTINGS.get("enable_txt_export"):
        ...
        write_to_file(os.path.join(out, "geo_ip_spoof.txt"), export_console.export_text())
