...
def banner():
    ...
    console.print(f"[cyan]     Perimetry – Hidden Parameter Discovery")
    console.print(f"[{TEAL}]" + "="*44 + "\n")
...
def run(target, threads, opts):
    ...
    baseline = get_baseline(base, timeout)
    if not baseline:
        console.print("[red][!] Could not fetch the baseline (connection refused or dropped).[/red]")
        return
    baseline_len, baseline_hash, baseline_status = baseline
