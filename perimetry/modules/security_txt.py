...
from perimetry.utils.util import clean_domain_input, clean_url, ensure_directory_exists, write_to_file, log_message, render_to_text
from perimetry.config.settings import DEFAULT_TIMEOUT, RESULTS_DIR, EXPORT_SETTINGS
...
def run(target, threads, opts):
    ...
    console.print(Fore.GREEN + "      Perimetry - Security.txt Detection")
    console.print(Fore.GREEN + "=============================================\n")
    console.print(f"[*] Fetching {url}\n")

    try:
        r = requests.get(url, timeout=timeout, verify=False, headers={"User-Agent": "Perimetry/1.0"})
    ...
    if EXPORT_SETTINGS.get("enable_txt_export"):
        ...
        write_to_file(os.path.join(out, "security_txt.txt"), render_to_text(table))
