...
from perimetry.utils.util import clean_domain_input, parse_runner_extras
...
def banner():
    ...
    console.print("[cyan]      Perimetry - Open Ports Scanner")
    console.print(f"[{TEAL}]" + "="*44)
...
def main():
    ...
    # Accept the runner's `<threads> [json-opts]` positionals alongside the flags.
    runner_threads, opts = parse_runner_extras(extra)
    if "ports" in opts:
        args.ports = str(opts["ports"])
    if "timeout" in opts:
        args.timeout = float(opts["timeout"])
    if "threads" in opts:
        runner_threads = int(opts["threads"])
    # A single worker would take hours over 1024 ports, so only let the runner
    # raise the concurrency, never drop it below the scanner's own default.
    if runner_threads and runner_threads > 1:
        args.threads = runner_threads
