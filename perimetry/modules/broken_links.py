...
def banner():
    ...
    console.print("[cyan]    Perimetry – Broken Links Detection")
    console.print("[cyan]" + "="*40)
...
def run(target, threads, opts):
    ...
    if not collected:
        console.print("[yellow][!] No links collected — target unreachable or served no HTML.[/yellow]")
        return
    pool_links = list(collected)
    sample_size = min(len(pool_links), max(5, int(len(pool_links) * ratio)))
    sample = random.sample(pool_links, sample_size)
    broken = []
