...
from perimetry.utils.util import clean_domain_input, ensure_directory_exists, write_to_file
from perimetry.config.settings import DEFAULT_TIMEOUT, EXPORT_SETTINGS, RESULTS_DIR
...
def banner():
    ...
    console.print("[cyan]  Perimetry – Dependency JS/CDN Scanner")
    console.print(f"[cyan]{bar}\n")
...
def run(target, threads, opts):
    ...
    if not urls:
        console.print("[yellow][!] No pages crawled — target unreachable or served no HTML.[/yellow]")
        return
    chosen = random.sample(urls, min(len(urls), max(1, int(len(urls) * ratio))))
    ...
    external_count = 0
    if not deps:
        console.print("[yellow][!] No linked resources found on the crawled pages.[/yellow]")
        return
    with Progress(
        SpinnerColumn(),
    ...
    if EXPORT_SETTINGS.get("enable_txt_export"):
        ...
        export_console = Console(record=True, file=io.StringIO(), width=console.width)
