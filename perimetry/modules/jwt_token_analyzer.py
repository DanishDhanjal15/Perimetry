...
from perimetry.utils.util import parse_runner_extras
...
def main():
    parser = argparse.ArgumentParser(description="Perimetry – JWT Token Analyzer")
    ...
    _, opts = parse_runner_extras(extra)
    if opts.get("jwks"):
        args.jwks = opts["jwks"]
    if opts.get("timeout"):
        args.timeout = int(opts["timeout"])
    if opts.get("token"):
        args.input = str(opts["token"])
