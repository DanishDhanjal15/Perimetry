...
from perimetry.cli.views.table_modules import module_info_table, display_table
from perimetry.cli.helpers import resolve_module_number, fuzzy_find_modules, regex_find_modules
from perimetry.core.catalog_cache import TOOL_TAGS, tools_mapping
...
class InfoMixin:
    ...
    def do_api(self, args) -> None:
        ...
        success = self._write_env_key(api_key_name, args.key)
        ...
        if success:
            ...
            console.print(Panel(header, expand=False, padding=(0, 2), style=TEAL))
            console.print()
            console.print(Text(f"API key for {service_name} saved to .env", style="bold green"))
            console.print()
        else:
            self.perror(f"Failed to save the API key for {service_name}")
            return
    ...
    def _resolve_service_name(self, service_input: str) -> str | None:
        expected_services = ["virustotal", "shodan", "google", "censys_key", "censys", "censys_secret", "hibp"]
    ...
    def _env_path(self) -> str:
        """Where settings.py looks for the dotenv file: the project root."""
        root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
        return os.path.join(root, ".env")

    def _write_env_key(self, api_key_name: str, api_key_value: str) -> bool:
        """Persist an API key to .env.

        Credentials go to .env - which is gitignored and already read by
        settings.py at import - and never into settings.py itself, which is
        tracked by git and would carry the secret into every commit.
        """
        value = api_key_value.strip()
        if "\n" in value or "\r" in value:
            console.print("API key cannot contain line breaks.", style="red")
            return False

        path = self._env_path()
        line = f"{api_key_name}={value}\n"
        try:
            lines: List[str] = []
            ...
            for i, existing in enumerate(lines):
                ...
                if not stripped or stripped.startswith("#") or "=" not in stripped:
                    continue
                if stripped.split("=", 1)[0].strip() == api_key_name:
                    lines[i] = line
                    replaced = True
                    break
            if not replaced:
            ...
            console.print(f"Error writing .env: {e}", style="red")
            return False

        # settings.py only reads .env at import time, so update the live process
        # too - otherwise the key would not take effect until the next launch.
    ...
    def _show_api_status(self) -> None:
        expected_services = ["virustotal", "shodan", "google", "censys_key", "censys", "censys_secret", "hibp"]
        display_names = {
            "censys_key": "Censys (Platform key)",
            "censys": "Censys ID (legacy)",
            "censys_secret": "Censys Secret (legacy)",
        }
        labels = [display_names.get(s, s.replace("_", " ").title()) for s in expected_services]
        svc_w = max(len(label) for label in labels) + 2
        ...
        for i, (service, display_name) in enumerate(zip(expected_services, labels), 1):
