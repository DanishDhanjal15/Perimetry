...
from perimetry.cli.status_bar import print_status_bar
...
_ENTRIES = [
    ("Module Browse", "modules [infra|web|sec|tag:<t>] [-d] [-s], search <k> [--regex|--exact]"),
    ("Select",        "use <id|name>, helpmod <id>, hm <id>"),
    ("Run",           "run [ids...] [target], runall <infrastructure|web|security|all>, runfav, last"),
    ("Options",       "set target v, set threads n, set opt v, set id opt v, unset opt, opts [id], show options_full"),
    ("Info",          "scope, recent, fav [add|del|run|list|clear|export|import|tag:<tag>], viewout [mod], grepout <re>"),
...
_CMD_DOCS = {
    ...
    "search":   {"usage": "search <keyword> [--regex] [--exact] [--case-sensitive]",
                 "desc":  "Search module names, descriptions and tags. --regex treats the keyword as a "
                          "regular expression, --exact matches the name substring only.",
                 "ex":    ["search ssl", "search takeover", "search '(?i)dns.*brute' --regex"]},
    "use":      {"usage": "use <id|name>",
                 "desc":  "Select a module as context. Subsequent opts/show/run without IDs target this module. "
                          "Right after a search, a bare number picks that search result instead of the module ID.",
                 "ex":    ["use 42", "use subdomain_enum_pro"]},
    "helpmod":  {"usage": "helpmod <id>   (alias: hm)",
                 "desc":  "Show full details for a module: section, description, target type and its options.",
                 "ex":    ["helpmod 7", "hm 154"]},
    "run":      {"usage": "run [ids...] [target] [--dry-run] [--stop-on-error]",
                 ...
                 "ex":    ["run 5 9 example.com", "run example.com", "run", "run 3 17 --dry-run"]},
    "runall":   {"usage": "runall <infrastructure|web|security|all> [--stop-on-error]",
                 "desc":  "Run every module in a family. 'all' runs the whole catalog - expect it to take a while.",
                 "ex":    ["runall infrastructure", "runall security"]},
    "runfav":   {"usage": "runfav",
                 "desc":  "Run every favorite module.",
                 "ex":    ["runfav"]},
    "last":     {"usage": "last",
                 "desc":  "Re-run the modules from the previous run.",
                 "ex":    ["last"]},
    ...
    "api":      {"usage": "api [service|id] [key]",
                 "desc":  "Show API status, or save an API key for a service. Keys are written to .env "
                          "(gitignored) and take effect immediately. Supports service names or IDs.",
...
class HelpMixin:
    ...
    def do_help(self, arg: str) -> None:
        ...
        console.print(_panel_title("Perimetry CLI Help"))
        console.print()
