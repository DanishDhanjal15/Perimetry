...
import argparse
from cmd2 import with_category, with_argparser, Cmd2ArgumentParser
...
from perimetry.cli.views.table_modules import display_table
from perimetry.cli.logo import logo, TEAL
from perimetry.core.catalog_cache import number_of_modules, VERSION, AUTHOR, ORIGINAL_AUTHOR
...
class UtilityMixin:
    ...
    def do_info(self, _line) -> None:
        console.print()
        console.print(_header(" Perimetry Info "))
        console.print()
        tbl = Table(box=SIMPLE_HEAVY)
        tbl.add_column("Attr", style="cyan", no_wrap=True)
        tbl.add_column("Value", style="green")
        tbl.add_row("Version", VERSION)
        tbl.add_row("Maintainer", AUTHOR)
        tbl.add_row("Original author", f"{ORIGINAL_AUTHOR} (Argus)")
    ...
    def do_exit(self, _line) -> bool:
        console.print(Text("Exiting Perimetry. Goodbye!", style="bold green"))
        return True
