...
from perimetry.cli.commands import register_mixins
from perimetry.cli.status_bar import print_status_bar
from perimetry.cli.logo import logo
from perimetry.cli.helpers import (
...
from perimetry.utils.util import check_api_configured
...
class _BareCLI(Base):
    ...
    def __init__(self):
        ...
        self.api_status = {
            ...
            "censys_key": check_api_configured("CENSYS_API_KEY"),
...
PerimetryCLI = register_mixins(_BareCLI)

def main():
    # The banner and every rich table use box-drawing and status glyphs. When
    # stdout is a pipe or a file, Windows falls back to cp1252 and the very
    # first print raises UnicodeEncodeError, killing the CLI before it starts.
    # perimetry.modules applies the same guard for module subprocesses.
    for stream in (sys.stdout, sys.stderr):
        try:
            stream.reconfigure(encoding="utf-8", errors="replace")
        except (AttributeError, ValueError):
            pass
    PerimetryCLI().cmdloop()
