...
def banner():
    console.rule("[bold cyan]Perimetry – Recon Swiss‑Army Knife", style="cyan")
...
_DISABLED_FILE = pathlib.Path.home() / ".perimetry_disabled"
...
def enable_module(module_id):
    ...
    _DISABLED_FILE.write_text(json.dumps(list(disabled)))
def check_updates(current_version: str, repo="DanishDhanjal15/perimetry"):
    try:
...
console = make_console(os.getenv("PERIMETRY_THEME", "default"))